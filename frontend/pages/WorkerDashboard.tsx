            ...
            <div className="bg-card-light dark:bg-card-dark p-6 rounded-2xl shadow-lg">
                <div className="space-y-4">
                    {myTasks.length > 0 ? myTasks.map(task => (
                        <div key={task.id} className="p-4 border border-border-light dark:border-border-dark rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-shadow hover:shadow-md">
                            <div>
                                ...
                                <div className="flex space-x-2 flex-shrink-0">
                                    {task.status === 'Pending' && (
                                        <button 
                                            ...
                                            className="px-4 py-2 rounded-md bg-blue-500 text-white hover:bg-blue-600 transition-colors text-sm font-semibold shadow"
                                        >
                                            Start Task
                                        </button>
                                    )}
                                    {task.status === 'In Progress' && (
                                         <button 
                                            ...
                                            className="px-4 py-2 rounded-md bg-green-500 text-white hover:bg-green-600 transition-colors text-sm font-semibold shadow"
                                        >
                                            Mark as Completed
                                        </button>
                                    )}
                        ...
                        <p className="text-center text-slate-500 dark:text-slate-400 py-8">You have no assigned tasks.</p>
                    )}
                </div>
            </div>
        </div>
